...
type Props = {
  activeAction: ActiveAction; input: ScenarioInput; selected: SelectedGridElement; presets: ApiDemoPreset[];
...
export function ScenarioControls({ activeAction, input, selected, presets, onLoadPreset, onProfileChange, onConditionChange, onClear, onPredict, onFailure, onCascade, onMitigation }: Props) {
  ...
  return <aside className="control-rail" aria-label="Scenario controls">
    <section><h2 className="eyebrow">Scenario</h2>
      <label className="field">Preset<div className="select-wrap"><select disabled={busy} value={input.presetId ?? ""} onChange={(event) => { const preset = presets.find((item) => item.id === event.target.value); if (preset) onLoadPreset(preset); }}><option value="" disabled>Custom scenario</option>{presets.map((preset) => <option value={preset.id} key={preset.id}>{preset.name}</option>)}</select><ChevronDown aria-hidden="true" /></div></label>
